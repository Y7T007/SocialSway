...
<div class="container">
	...
	<?php
        ...
        if (isset($_GET['id_contrat']) && isset($_SESSION['id'])) {
            $stm = $conn->prepare('SELECT * FROM contrat WHERE id_contrat=:id_contrat');
            $stm->bindParam(":id_contrat", $_GET['id_contrat']);
            $stm->execute();
            ...
            $result->bindParam(":id", $rows['id_influenceur']);
            $result->execute();
...
<div class="item_f">
    ...
    <div class="sign">
		  <div>
			...
			<img width="100px" src="image/<?php echo $rows['signature_marque'] ?>" alt="logo">
			</div>
			<div>
				<?php 

			if (empty($rows['signature_influenceur'])) {
			if ($_GET['type'] != 'marque') {   // Display the form to add the signature
				?>
				<form action="" method="post" enctype="multipart/form-data">
				<input type="hidden" name="contrat_id" value="<?php echo $row['id_contrat'] ?>">
				...
				$query = "UPDATE contrat SET signature_influenceur = :signature_path WHERE id_contrat = :contrat_id and id_influenceur=:id_influenceur and id_marque=:id_marque";
